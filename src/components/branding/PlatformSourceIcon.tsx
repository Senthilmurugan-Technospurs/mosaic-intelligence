...
const GOOGLE_ADS_SRC =
  'https://www.gstatic.com/images/branding/product/2x/ads_48dp.png';

function MetaIcon({ size }: { size: number }) {
  const gid = `meta-grad-${size}`;
  return (
    <svg width={size} height={size} viewBox="0 0 287.56 191" aria-hidden>
      <defs>
        <linearGradient id={gid} x1="62.6" y1="160" x2="237.5" y2="32" gradientUnits="userSpaceOnUse">
...
function MosaicIcon({ size }: { size: number }) {
  return (
    <span
      className="inline-flex shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#22c7ee] to-[#0ea5c6] text-white shadow-sm"
...
const SOURCE_TITLE: Record<RecommendationSource, string> = {
  mosaic: MOSAIC_PRODUCT_NAME,
  google_ads: 'Google Ads',
  meta: 'Meta',
};

export default function PlatformSourceIcon({
  source = 'mosaic',
  size = 20,
  title,
}: {
  ...
  const label = title ?? SOURCE_TITLE[source];
  ...
  if (source === 'meta') {
    return <span title={label}><MetaIcon size={size} /></span>;
  }

  return <span title={label}><MosaicIcon size={size} /></span>;
}
